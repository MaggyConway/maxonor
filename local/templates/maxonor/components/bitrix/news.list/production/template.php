...
{
	$arIdsSections[$arItem['IBLOCK_SECTION_ID']] = $arItem['IBLOCK_SECTION_ID'];
}
?>
...
<section class="production">
	<div class="wrapper">
		...
		<!-- <span class="warranty">7&nbsp;лет гарантия</span> -->
		...
		if (is_array($arIdsSections) && count($arIdsSections) > 0)
		{
			$resSectList = CIBlockSection::GetList(
				...
				false,
				array("ID", "NAME", "CODE", "UF_SHORTNAME"),
				false
			);

			while ($arSectList = $resSectList->GetNext()) {
				$sectionsList[$arSectList['ID']] = $arSectList;
			}
		}
		?>
		...
		<div class="tabs_parent">
			<ul class="tabs">
				...
				<?if(is_array($sectionsList) && count($sectionsList) > 0):?>
					<? foreach ($sectionsList as $key => $sect) : ?>
						<li data-code="<?= $sect['CODE'] ?>"><?= $sect['NAME'] ?></li>
					<? endforeach; ?>
				<?endif;?>
			</ul>
		</div>
				...
				?>
					...
					<div class="production__gallery__map" data-code="<?= $sectionsList[$arItem['IBLOCK_SECTION_ID']]['CODE']?>">
						...
						<? if (isset($arItem["PROPERTIES"]["PRODUCT"])) :
							...
							<a href="<?= $product_url ?>" class="production__gallery__tag" target="_blank">
								Смотреть товар
							</a>
		...
		<a href="javascript:void(0);" class="gallery_show_more">Загрузить&nbsp;еще</a>
	</div>
</section>